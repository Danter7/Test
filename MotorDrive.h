...
//---------------------------------------------- DRIVE Motors I/O Pins ---------------------------------
    #define DriveMotor_Left_Frwd_Pin   26  //-- [Forward] GPIO pin
    #define DriveMotor_Left_Bkwd_Pin   25  //-- [Backward] GPIO pin
    #define DriveMotor_Right_Frwd_Pin  32
    #define DriveMotor_Right_Bkwd_Pin  33
...
//---------------------------------------IR Distance Sensor Variables-----------------------------------
    #define IR_DistanceSensorPin 17    //-- input pin for the IR distance sensor
//\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
...
void IRAM_ATTR IR_LedgeDetector_ISR();
...
{  
  public:
    ...
    void Go()
    {
      ...
      //attachInterrupt(  IR_DistanceSensorPin,               IR_LedgeDetector_ISR,               RISING);    //-- attach & activate the IR ledge sensor
      Running = true;
    ...
    void CheckEnd()
    {
      ...
      int CurrentRevolutions_Left = DriveMotor_Left_RotaryEncoder.Count;    ///7)/603);   //-- convert revolutions count to the number of wheel revolutions
      int CurrentRevolutions_Right = DriveMotor_Right_RotaryEncoder.Count;

      if(CurrentRevolutions_Left >= TargetRevolutions || CurrentRevolutions_Right >= TargetRevolutions)
      {
        if(CurrentRevolutions_Left >= TargetRevolutions)
        {
          Done_Left = true;
          ledcWrite(DriveMotor_Left_Frwd_PwmChannel, 0);
          detachInterrupt(DriveMotor_Left_RotaryEncoder.PIN);
        }
        if(CurrentRevolutions_Right >= TargetRevolutions)
        {
          Done_Right = true;
          ledcWrite(DriveMotor_Right_Frwd_PwmChannel, 0);
      ...
      if(Done_Left == true && Done_Right == true)
      {
        detachInterrupt(IR_DistanceSensorPin);                //-- detach & deactivate the IR ledge sensor
        Running = false;  Done_Left = false;  Done_Right = false;
        AllMotorsOFF();
...
class CLASS_PathPointTurn_Left90
{  
  public:
...
class CLASS_PathPointTurn_Right90
{  
  public:
...
CLASS_PathPointTurn_Left90  PathPointTurn_Left90;
CLASS_PathPointTurn_Right90 PathPointTurn_Right90;
...
//---------------------------------------------- DRIVE Motor ISRs --------------------------------------
    ...
    void CheckMovementEnds()
    {
      ...
      else if(PathPointTurn_Left90.Running == true)
        PathPointTurn_Left90.CheckEnd();

      else if(PathPointTurn_Right90.Running == true)
        PathPointTurn_Right90.CheckEnd();
    }
//\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\

//######################################################################################################
//--------------------------------IR Ledge detection InterruptServiceRoutine ISR------------------------
    void IRAM_ATTR IR_LedgeDetector_ISR()
    {
      //-- ### emergency stop the drive motors. ###
      if(PathMove_Forward.Running == true)  //-- if we are currently moving forward, then check when to stop moving
        PathMove_Forward.EmergencyEnd();

      else if(PathMove_Backward.Running == true) //-- if we are currently moving backward, then check when to stop moving
        PathMove_Backward.EmergencyEnd();

      else if(PathArcTurn_Left90.Running == true)
        PathArcTurn_Left90.EmergencyEnd();

      else if(PathArcTurn_Right90.Running == true)
        PathArcTurn_Right90.EmergencyEnd();
        
      else if(PathPointTurn_Left90.Running == true)
        PathPointTurn_Left90.EmergencyEnd();

      else if(PathPointTurn_Right90.Running == true)
        PathPointTurn_Right90.EmergencyEnd();
